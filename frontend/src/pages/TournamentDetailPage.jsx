...
const TournamentDetailPage = () => {
  ...
  const tournamentPhase = getTournamentPhase(tournament);
  const isRegistrationPhase = tournamentPhase === 'registration';
      ...
      <section className="relative text-white py-16">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            ...
            <p className="text-xl mb-8 text-white/90">{tournament.description || 'Giải đấu chưa có mô tả.'}</p>
            ...
            <div className="flex items-center justify-center space-x-4 mb-6 text-sm">
              ...
              <span className="text-white/50">||</span>
              <span>{tournament.location || 'Địa điểm'}</span>
            </div>
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              {activeTab === 'registration' && (
                <div className="bg-gray-900 rounded-xl shadow-lg p-6 border border-gray-700">
                  <h3 className="text-xl font-bold text-white mb-6">Đăng ký thi đấu</h3>
                  <div className="text-center py-8">
                    <div className="mb-6">
                      <h4 className="text-lg font-semibold text-white mb-4">Thông tin đăng ký</h4>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-left">
                        <div className="bg-gray-800 rounded-lg p-4">
                          <p className="text-gray-400 text-sm">Hạn đăng ký:</p>
                        ...
                        <div className="bg-gray-800 rounded-lg p-4">
                          <p className="text-gray-400 text-sm">Số đội đã đăng ký:</p>
                          <p className="text-white font-semibold">
                            {stats.totalTeams || tournament.Registrations?.length || 0}/{tournament.max_teams || 0}
                          </p>
                        </div>
                        <div className="bg-gray-800 rounded-lg p-4">
                          <p className="text-gray-400 text-sm">Số thành viên tối thiểu:</p>
                          <p className="text-white font-semibold">{tournament.min_players_per_team || 2} người</p>
                        </div>
                        <div className="bg-gray-800 rounded-lg p-4">
                          <p className="text-gray-400 text-sm">Số thành viên tối đa:</p>
                          <p className="text-white font-semibold">{tournament.max_players_per_team || 20} người</p>
                        </div>
                      </div>
                    </div>
                    ...
                    {/* Countdown Timer */}
                    <div className="mb-6">
                      <p className="text-gray-400 mb-4">Thời gian còn lại để đăng ký:</p>
                      <div className="flex justify-center space-x-4">
                        <div className="bg-gray-800 rounded-lg p-4 min-w-[80px]">
                          <div className="text-2xl font-bold text-[#30ddff]">{timeLeft.days}</div>
                          <div className="text-sm text-gray-400">Ngày</div>
                        </div>
                        <div className="bg-gray-800 rounded-lg p-4 min-w-[80px]">
                          <div className="text-2xl font-bold text-[#30ddff]">{String(timeLeft.hours).padStart(2, '0')}</div>
                          <div className="text-sm text-gray-400">Giờ</div>
                        </div>
                        <div className="bg-gray-800 rounded-lg p-4 min-w-[80px]">
                          <div className="text-2xl font-bold text-[#30ddff]">{String(timeLeft.minutes).padStart(2, '0')}</div>
                          <div className="text-sm text-gray-400">Phút</div>
                        </div>
                        <div className="bg-gray-800 rounded-lg p-4 min-w-[80px]">
                          <div className="text-2xl font-bold text-[#30ddff]">{String(timeLeft.seconds).padStart(2, '0')}</div>
                    ...
                    <Link 
                      ...
                      className="inline-block bg-green-500 hover:bg-green-600 text-white px-8 py-4 rounded-lg text-xl font-bold transition-colors"
                    >
                      Bắt đầu đăng ký
                    </Link>
                  </div>
              ...
              {activeTab === 'registration-list' && (
                <div className="bg-gray-900 rounded-xl shadow-lg p-6 border border-gray-700">
                  <h3 className="text-xl font-bold text-white mb-6">Danh sách đăng ký</h3>
                  {(tournament.Registrations && tournament.Registrations.length > 0) ? (
                    <div className="space-y-4">
                      {tournament.Registrations.map((reg, index) => (
                        <div key={reg.id} className="bg-gray-800 rounded-lg p-4 border border-gray-700">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center">
                              <span className="w-8 h-8 bg-[#30ddff] text-white flex items-center justify-center rounded-full font-bold mr-3">
                                {index + 1}
                              </span>
                              <div>
                                <p className="font-semibold text-white">{reg.Team?.name || 'Đội chưa xác định'}</p>
                                <p className="text-xs text-gray-400">{reg.Team?.university || 'N/A'}</p>
                              </div>
                            </div>
                            <div className="text-right">
                              <p className="text-sm text-gray-400">Đăng ký lúc:</p>
                    ...
                    <div className="text-center py-8">
                      <p className="text-gray-400 mb-4">Chưa có đội nào đăng ký.</p>
              ...
              {activeTab === 'general' && (
                <>
                  <div className="bg-gray-900 rounded-xl shadow-lg p-6 border border-gray-700">
                    <h3 className="text-xl font-bold text-white mb-6">Thông tin giải đấu</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                        <span className="text-sm font-medium text-gray-400">Loại giải:</span>
                        <p className="text-white mt-1">{tournament.Tournament_Type?.name || 'Chưa xác định'}</p>
                      </div>
                      <div>
                        <span className="text-sm font-medium text-gray-400">Địa điểm:</span>
                        <p className="text-white mt-1">{tournament.location || 'Chưa xác định'}</p>
                      </div>
                      <div>
                        <span className="text-sm font-medium text-gray-400">Ngày bắt đầu:</span>
                        <p className="text-white mt-1">{formatDate(tournament.start_date)}</p>
                      </div>
                      <div>
                        <span className="text-sm font-medium text-gray-400">Ngày kết thúc:</span>
                        <p className="text-white mt-1">{formatDate(tournament.end_date)}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-gray-900 rounded-xl shadow-lg p-6 border border-gray-700">
                    ...
                    {(matches && matches.length > 0) ? (
                      ...
                      <p className="text-gray-400 text-center py-8">Chưa có lịch thi đấu.</p>
                    )}
                  </div>
                </>
              )}
              ...
              {activeTab === 'schedule' && (
                <div className="bg-gray-900 rounded-xl shadow-lg p-6 border border-gray-700">
                  <h3 className="text-xl font-bold text-white mb-6">Vòng bảng</h3>
              ...
              {activeTab === 'standings' && (
                <div className="bg-gray-900 rounded-xl shadow-lg p-6 border border-gray-700">
                  ...
                  <p className="text-gray-400 text-center py-8">Chưa có bảng xếp hạng.</p>
                </div>
              )}

              {activeTab === 'teams' && (
                <div className="bg-gray-900 rounded-xl shadow-lg p-6 border border-gray-700">
                  <h3 className="text-xl font-bold text-white mb-6">Đội thi đấu</h3>
                  {(tournament.Registrations && tournament.Registrations.length > 0) ? (
                    ...
                    <p className="text-gray-400 text-center py-8">Chưa có đội nào đăng ký.</p>
                  )}
                </div>
              )}
